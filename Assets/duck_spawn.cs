...
public class DuckSpawner : MonoBehaviour
{
    ...
    private void EndGame()
    {
        ...
        SceneManager.LoadScene("Scenes/GameOver"); // Load the end screen scene
    }
    ...
    IEnumerator SpawnDucksSequentially()
    {
        for (int i = 0; i < duckCount; i++)
        {
            ...
            // Attach movement script to the duck
            DuckMovement duckMovement = duck.AddComponent<DuckMovement>();
            duckMovement.speed = duckSpeed;
            duckMovement.yEnd = 50f;
            duckMovement.zBounds = new Vector2(11f, 36f);

            // Attach collision script and link score increment
            DuckCollision duckCollision = duck.AddComponent<DuckCollision>();
            duckCollision.OnDuckHit = AddScore; // Pass the AddScore method
            duckCollision.OnDuckDestroyed = OnDuckDestroyed; // Track duck destruction
